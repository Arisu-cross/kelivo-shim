...
| `WAKE_FREE` / `WAKE_FREE_EVERY` / `WAKE_ACTIVITIES` | 开 / `3` / 空 | 自主时间里「做点自己的事」这一档,见 §7.1 |
...
### 7.1 让它不只是「说话或沉默」——自己找点事做

自主时间走的是和普通对话**完全相同**的管道:同一个常驻进程、同一份人设、同一套 MCP 工具。
也就是说 AI 在这一轮里**本来就能**调工具(记忆/游戏/社交/浏览器/搜索),从来没有东西拦着它——
缺的只是没人告诉它「这轮也可以拿来做自己的事」。

所以每 `WAKE_FREE_EVERY` 次唤醒会给一次**宽版**提示,把「做点自己的事」写成与说话、
沉默**并列**的第三个选项;其余轮次的提示词与旧版逐字节相同。

- **选项平权,不是派活**:宽版末尾明说「没有哪个更应该」,并告诉它做完不必汇报。
  把「必须说话」换成「必须干活」是同一个错误。
- **可做的事按 `ALLOWED_TOOLS` 自动推导**,不会撺掇它用一个不存在的工具;
  想自己写就设 `WAKE_ACTIVITIES`(用 `/` `、` `|` 分条)。
- **为什么不每轮都宽**:静默续命是全天最便宜的一轮。每轮都摊开菜单等于把最便宜的
  那一轮变贵,窗口也涨得更快 → 更早触发自动归档与压缩。默认每 3 次一回(开窗后第一轮就宽,
  它不用等三轮才知道自己可以)。
- **它真做了事的那一轮不算空轮**:产出留在窗口里,压缩会吃掉它,所以压缩闸门会照拦
  (只调记忆工具的除外——那些内容本来就写在记忆库里,不怕压)。
- **看得见**:`/debug` 的 `wake.free` 报开关、节奏、可做的事,以及它上次动手的时刻和
  用了哪些工具(**只记工具名,不记参数也不记返回**)。

| 变量 | 默认 | 说明 |
|---|---|---|
| `WAKE_FREE` | 开 | 设 `0` 完全退回旧的「说话 or 沉默」二选一 |
| `WAKE_FREE_EVERY` | `3` | 每几次唤醒给一次宽版 |
| `WAKE_ACTIVITIES` | 空 | 显式指定可做的事,不填则按 `ALLOWED_TOOLS` 推导 |

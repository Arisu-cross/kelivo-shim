...
| `SYSTEM_PROMPT_MODE` | `append`(默认) | `append` = 保留 claude -p 自带的系统提示词,再追加锚点(历史行为);`replace` = **整段换掉**那份自带的编程代理提示词,用本项目自己的正文。实测(CLI 2.1.239):替换后 CLAUDE.md 照常加载、MCP 工具与钩子不受影响,常驻前缀从 ~7000 token 降到 ~2200。⚠️ 改这个 = 进程重启 = 对话窗口重置,先安排归档 |
| `SYSTEM_PROMPT` | (可选)覆盖 replace 模式的内置正文 | 只在 `replace` 模式生效 |
| `SYSTEM_PROMPT_FILE` | `system-prompt.md` | `replace` 模式的正文文件(相对 `/src`)。正本放持久卷 `/persona`,开机由人设保险箱复印过来,所以可以写私人内容。**文件不在就自动退回内置正文**,不会让子进程起不来 |
...
| 比官方端疏远/解离,人设"读了没内化",先摆事实给方案然后赶人 | claude -p 的内置系统提示把身份钉成"编程 CLI 助手",CLAUDE.md 以"须遵守的项目指令"姿态注入 → 人设被当扮演要求合规执行而非"我自己";且 CC 的训练目标就是"简洁、解决、结束回合" | 代码已带 SOUL_ANCHOR 锚点(追加在系统提示词末尾,措辞可 env 覆盖);人设本体尽量**第一人称**写("我是…"而非"你要扮演…");`THINK_EFFORT` 提到 `medium` 给他思考里"进入自己"的余地也有帮助。**更彻底的做法**:`SYSTEM_PROMPT_MODE=replace` 直接把那份内置提示词换掉——锚点是"先被定性错再抢回来"的对拉,替换是一开始就没被定性错。代价是内置提示词里那些"简洁、别跑题"的约束一并没了,行为会有漂移,切过去后观察几天 |

...
| `PERSONA_TGZ_B64` | (强烈建议)私有文件打包 | **重部署自愈**:`CLAUDE.md`/`你的人设.md`/`gmail-auth/` 都在 .gitignore 里,任何干净重部署(git 自动部署、新沙盒上传)都会把它们弄丢 → 人设消失、只剩 WebSearch/WebFetch。把 `tar czf - CLAUDE.md 你的人设.md gmail-auth 2>/dev/null \| base64 -w0` 的输出存进这个变量(env 跨部署持久),每次开机自动解包恢复;盘上已有的文件优先,不会盖掉新上传的 |
| `MCP_JSON` | (强烈建议)`.mcp.json` 原文 | 同上:`.mcp.json` 也是 gitignore 的,丢了之后 entrypoint 只会生成占位符 → 记忆/工具 MCP 全部失联。把真实 `.mcp.json` 的 JSON 原文存进来即可 |

> **⚠️ 重部署会丢私有文件**:上表最后两个变量就是保险丝。没配的话,每次重部署后**必须**手动把
> `CLAUDE.md`、`你的人设.md`、`.mcp.json`、`gmail-auth/` 重新传上去,否则 AI 会"失忆+失格"。
> 出过事故的判断口径:`GET /health` 返回里 `persona:false` 或 `mcp:"placeholder"` = 私有文件丢了;
> 应急恢复走 Zeabur 面板 → 该服务 → Deployments → 找上一个正常的部署 → Redeploy(旧镜像里私有文件还在)。
...
| 重部署后人设消失、AI 说只有 WebSearch/WebFetch、记忆工具全没 | 干净重部署不带 gitignore 的私有文件:CLAUDE.md/你的人设.md/.mcp.json 丢了,entrypoint 重新生成的 .mcp.json 是占位符 | ①应急:Zeabur 面板 → Deployments → 回滚上一个正常部署(旧镜像里文件还在);②重传私有文件;③一劳永逸:配 `PERSONA_TGZ_B64` + `MCP_JSON` 环境变量(见 §3.6),开机自愈。`GET /health` 的 `persona`/`mcp` 字段一眼确认 |

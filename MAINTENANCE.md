...
- **表情回应**(`reactions.js`):回复里的 `[回应:❤️]` → `setMessageReaction`
  贴在**使用者刚发的那条消息**上,不新增消息、不打断节奏。三处别改坏:
  ①**Telegram 只认一张固定白名单**(`ALLOWED`,照抄官方名单),名单外的表情
  API 直接回 400 `REACTION_INVALID` —— 模型很自然会想写 😏,而它不在名单里,
  所以白名单是前提不是可选校验;②**一轮只贴一个**,`setMessageReaction` 是「设置」
  不是「追加」,第二次调用会把第一次顶掉 = 前一个表情凭空消失;③贴不上的一律
  **降级成一条只有表情的气泡**(没有触发消息的心跳/主动开口、名单外、TG 拒收),
  不静默丢、也不把 `[回应:…]` 标记漏给使用者看。标记里不像表情的内容(`[回应:好的]`)
  原样当文字留着,同贴纸对未知名字的处理。
- **行踪上报 + 查岗**(`check.js`):手机快捷指令 `GET /report?key=…&app=…` 把「她打开了某个 App」
  攒进内存(48h/300 条,不落盘、不打日志);AI 在回复里写 `[查岗]` → 出口剥掉标记 →
  查一份摘要作为**新一轮**`【系统·查岗】`喂回去 → 他再决定说不说(回`【沉默】`即不打扰)。
  三处别改坏:①**结果轮 `kind=lookup` 不再响应标记**,否则无限打转;
  ②查岗轮**不走 `submitTurn`**、直接 `enqueue` —— 他自己伸头看一眼**不等于使用者回来了**,
  刷了 `lastUserAt` 会让他之后说的等待时长全是假的;③空轮(回`【沉默】`)和心跳轮一样
  **不置 `dirty`**,否则几条查岗就能把压缩闸门一直拉着。钥匙 `REPORT_TOKEN` **与 `SHIM_KEY` 分开**
  (这把要存进使用者手机里),不设即整套静默关闭。细节与踩坑见
  [docs/手机行踪上报与查岗.md](docs/手机行踪上报与查岗.md)。
...
`package.json` 里 `@anthropic-ai/claude-code` **写的是精确版本号,没有 `^`**。
这是有意的,别顺手改回 caret。

- 这个依赖就是 AI 本体。放开范围 = **每次重新构建都可能换一个 CLI 版本**,
  而换版本的后果不一定报错:上面那条钩子契约就是「变了不报错,只会安静失效」;
  社区也报过某些版本会把特定模型的 thinking 内容削空(思考链直接没了,但服务照跑)。
- 同类事故有前科:另一处依赖曾写成无上限范围,上游一发新大版本,当天构建直接拉崩。
- 运行期再加一道:部署环境里设 `DISABLE_AUTOUPDATER=1`,防止容器起来后 CLI 自更新。

**升级流程**:先查清目标版本改了什么 → 改这里的版本号 → 部署 → 按下面「工作规范」
主动验证(尤其思考链和压缩闸门这两处最容易安静失效)。**别在没验证的情况下追最新版。**

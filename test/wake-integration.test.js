...
before(async () => {
  ...
  proc = spawn("node", [SERVER], {
    env: {
      ...
      ALLOWED_TOOLS: "WebSearch,WebFetch,mcp__ombre,mcp__galatea",
      FAKE_WAKE_TOOL: "mcp__galatea__post",   // 演成他去社交圈发了点东西
...
test("/debug 报得出这一档的状态,活动清单按 ALLOWED 推导", async () => {
  ...
  assert.ok(f.activities.some((x) => x.includes("记忆")));
...
test("【核心】他在自主时间里做了事 → 记下工具名,且不算空轮(压缩要被拦下)", async () => {
  ...
  const d = await dbg();
  assert.deepEqual(d.wake.free.lastTools, ["mcp__galatea__post"], "该记下他用了社交圈工具");
  assert.ok(d.wake.free.lastActedAt, "该记下他动手的时刻");
  assert.equal(d.gate.dirty, true, "做了事的那一轮不是空轮,闸门必须拦压缩,否则他做的事会被压没");
  assert.ok(logs.includes("[wake] did: mcp__galatea__post"), logs);
});
...
test("【载荷】宽版真的把三个选项和活动清单喂了进去,不是只走了分支", async () => {
  ...
  assert.ok(recv.includes("社交圈"), "活动清单该真的进到文本里:" + recv);
...
test("只记工具名,不记参数也不记返回", async () => {
  ...
  assert.ok(s.includes("mcp__galatea__post"));
